...
void testPreviousNext(TList *list)
{
    ...
    do
    {
        if (node != list->HEAD)
        {
            CU_ASSERT_PTR_EQUAL(node->previous, get(list, i - 1));
        }
    ...
    } while (node != NULL && list->HEAD != node);

    CU_ASSERT_PTR_EQUAL(node, list->HEAD);
    CU_ASSERT_PTR_EQUAL(node->previous, get(list, list->size - 1));
}
...
void testRemoveStart()
{
    ...
    CU_ASSERT_PTR_NOT_EQUAL(list->HEAD, NULL);
...
void testInsertStart()
{
    ...
    CU_ASSERT_EQUAL(list->HEAD->data.data, 2);
    ...
    CU_ASSERT_EQUAL(list->HEAD->data.data, -1);
...
void testInsertEnd()
{
    ...
    CU_ASSERT_EQUAL(list->HEAD->data.data, 0);
    CU_ASSERT_EQUAL(get(list, 0)->data.data, 0);
    CU_ASSERT_EQUAL(get(list, 1)->data.data, 1);
